...
struct ColumnHeader
{
	...
	float Size = -1.0f; // If size is negative, will scale based on column header text
	std::function<void(void)> OnClick = nullptr;
	bool IsSetup = false;
};

IMGUI_API void BeginColumnTable(const char* columnsId, ImVec2 size, ColumnHeader* headers, int count, bool border = true);
IMGUI_API void EndColumnTable(int count);
